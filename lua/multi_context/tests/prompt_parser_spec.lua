...
-- O registry acessa arquivos de skill em disco, indisponível no ambiente mínimo.
-- Stub injetado antes do require para isolar o módulo testado.
package.loaded['multi_context.skills.registry'] = {
    build_manual_for_skills = function(skills)
        return "=== FERRAMENTAS DO SISTEMA ===\n" .. table.concat(skills, ", ")
    end
}

-- Força reload limpo do prompt_parser para ele pegar o stub acima
...
describe("Prompt Parser Module:", function()
    local mock_agents = {
        coder = {
            system_prompt = "Você programa.",
            skills = { "read_file", "edit_file", "run_shell" }
        }
    }
