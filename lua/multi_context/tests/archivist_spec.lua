...
describe("Fase 22 - Passo 3: A Persona @archivist e a Compressao", function()
    ...
    before_each(function()
        ...
        StateManager.get('react').pending_user_prompt = "Este e o meu comando original"
        StateManager.get('react').active_agent = "archivist"
    ...
    it("Deve extrair o XML Quadripartite, limpar o buffer e re-anexar o prompt pendente", function()
        ...
        assert.is_nil(StateManager.get('react').pending_user_prompt)
        assert.is_nil(StateManager.get('react').active_agent)
